...
\begin{tikzpicture}[
  >=Stealth, font=\sffamily\footnotesize, x=1cm, y=1cm,
  blk/.style={rectangle, draw=black!45, rounded corners=2pt, fill=white,
              align=center, inner sep=4pt, minimum height=8mm},
  sum/.style={circle, draw=black!55, fill=white, minimum size=6mm, inner sep=0pt},
  wire/.style={-Stealth, line width=0.8pt, draw=wiregray, rounded corners=2pt},
  obs/.style={-Stealth, line width=1.2pt, draw=deepblue, rounded corners=2pt},
  act/.style={-Stealth, line width=1.2pt, draw=accentred, rounded corners=2pt},
  lbl/.style={font=\sffamily\scriptsize, inner sep=1.5pt}]

  \begin{scope}[on background layer]
    \fill[panelblue, rounded corners=6pt] (-0.4,-0.6) rectangle (10.6,3.4);
    \fill[panelred,  rounded corners=6pt] (11.0,-0.6) rectangle (17.4,3.4);
  \end{scope}
  \node[lbl, anchor=west, text=deepblue] at (-0.3,3.15) {\bfseries A\ \ REFERENCE-BASED ANC PLANT (deployment: observables only)};
  \node[lbl, anchor=west, text=accentred] at (11.1,3.15) {\bfseries B\ \ META-RL CONTROLLER (ours)};

  % --- plant ---
  \node[blk] (src)  at (0.6,2.3) {clean\\ $d[n]$};
  \node[blk] (path) at (0.6,0.4) {unknown\\ path $H$};
  \node[blk, text=black!55] (isrc) at (-0.9,0.4) {ref.\\ $x[n]$};
  \node[sum] (p)    at (3.0,2.3) {$+$};
  \node[blk] (kal)  at (5.6,0.4) {\textbf{Kalman filter}\\ $\mathbf{w}_t,\,P_t$\ ($M{=}16$)};
  \node[sum] (s)    at (8.4,2.3) {$-$};

  \draw[wire] (src) -- (p) node[midway,above,lbl]{};
  \draw[wire] (isrc) -- (path);
  \draw[wire] (path) -| (p) node[near end,left,lbl]{$v[n]$};
  \draw[wire] (p) -- (s) node[midway,above,lbl]{primary $y[n]$};
  \draw[wire] (kal) -- (s) node[midway,left,lbl]{$\hat{v}[n]$};
  \draw[obs] (s) -- (10.2,2.3) node[midway,above,lbl]{$e[n]$ (denoised out)};
  % reference into filter
  \draw[wire] (isrc) |- (2.0,-0.2) -| (kal.south);
  \node[lbl] at (2.3,-0.4) {reference $x[n]$};
  % innovation feedback to filter
  \draw[obs] (9.3,2.3) |- (7.6,1.3) -- (kal.east);
  \node[lbl,text=deepblue] at (8.5,1.5) {adapt};

  % --- controller ---
  \node[blk] (phi)  at (12.3,2.3) {state $\bm{\phi}_t\!\in\!\mathbb{R}^{11}$\\ {\scriptsize innovation, $S_t$, gain, $\log Q_{t-1}$}};
  \node[blk] (lstm) at (15.6,2.3) {LSTM$_{256}\!\times\!2$\\ actor--critic};
  \node[blk] (out)  at (15.6,0.5) {$(Q_t,R_t)$\\ {\scriptsize process/meas.\ noise}};

  \draw[obs] (10.5,2.3) |- (11.0,2.3) -- (phi);
  \draw[wire] (phi) -- (lstm);
  \draw[act] (lstm) -- (out);
  \draw[act] (out) -| (5.6,-0.2) -- (kal.south west);
  \node[lbl,text=accentred] at (9.5,-0.4) {set $Q_t$ (process noise) $\to$ filter};
