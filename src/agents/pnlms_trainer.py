"""PNLMS BPTT+RL training engine — per-tap step sizes + functional expansion.

The key differences from hybrid_trainer:
  1. Action space is M+1 (per-tap mu + lambda) instead of 2
  2. Functional expansion of input (x, x^2, sign*sqrt|x|) inside filter
  3. Feature vector includes weight magnitude snippet (11 + M features)
  4. Same BPTT+RL training loop with auxiliary heads

Training loop (per iteration):
  Phase 1 — BPTT phase:
    Sample batch, run controller -> PNLMS, backprop truncated BPTT.
...
from ..filters.diff_pnlms import DiffPNLMSConfig, decode_action_pnlms, functional_expand
from ..signals.generators import make_signal
from ..noise.families import make_noise, TRAIN_FAMILIES, OOD_FAMILIES
from ..agents.pnlms_controller import PNLMSController


@dataclass
class PNLMSTrainConfig:
    n_iters: int = 6000
    batch_size: int = 16
...
def _sample_episode(rng: np.random.Generator, cfg: PNLMSTrainConfig,
...
def train_pnlms(cfg: PNLMSTrainConfig, out_dir: str = "results/v4_pnlms",
    ...
    if resume_from is not None and os.path.isfile(resume_from):
        ...
        print(f"[v4-PNLMS] Resumed from {resume_from} at iter {start_iter}")
        for _ in range(start_iter):
            if scheduler is not None:
                scheduler.step()

    print(f"[v4-PNLMS] Training PNLMS BPTT+RL on {device}")
    print(f"[v4-PNLMS] Controller params: {sum(p.numel() for p in controller.parameters()):,}")
    print(f"[v4-PNLMS] Action dim: {act_dim} (M={M} per-tap mu + 1 lambda)")
    print(f"[v4-PNLMS] Functional expansion: {'ON' if cfg.use_fx else 'OFF'} (order={cfg.fx_order})")
    ...
    for it in range(start_iter, cfg.n_iters):
        ...
        for t in range(T):
            ...
            grad_norm = torch.abs(last_mu_mean * e) / torch.sqrt(input_norm + 1e-8)
            ...
            action, state, value, pred_err, pred_sig, pred_task = controller(feat_t, state)

            mu_vec, lam = decode_action_pnlms(action[0], diff_cfg)
            ...
            w = lam.unsqueeze(1) * w + (mu_expanded / input_norm.unsqueeze(1)) * e.unsqueeze(1) * x_exp
        ...
        # === RL Phase (after warmup) ===
        if it >= cfg.rl_phase_start and cfg.rl_loss_weight > 0:
            _rl_phase_pnlms(controller, diff_cfg, cfg, rng, device, optimizer)
        ...
        if it % 50 == 0 or it == cfg.n_iters - 1:
            print(f"[v4-PNLMS] it={it:5d}  ss_mse_db={ss_mse_db:+.2f}  "
                  f"ep_mse_db={ep_mse_db:+.2f}  mu={mean_mu:.4f}  "
                  f"mu_sparsity={mu_sparsity:.4f}  lam={mean_lam:.4f}  "
        ...
        if it % cfg.eval_every == 0 and it > 0:
            _quick_eval_pnlms(controller, diff_cfg, cfg, device, rng)
    ...
    print(f"[v4-PNLMS] Done. Final model: {final_path}")
    return controller, records, final_path


def _rl_phase_pnlms(controller, diff_cfg, cfg, rng, device, optimizer):
    """PPO update for PNLMS controller with per-tap action space."""
...
def _quick_eval_pnlms(controller, diff_cfg, cfg, device, rng):
    from ..signals.generators import make_signal
    ...
    for fam in test_families:
        ...
        for t in range(N):
            ...
            autocorr = np.clip(e * last_e / (ema_e2 + 1e-8), -1.0, 1.0)
            ...
            feat = torch.cat([
                torch.tensor([np.tanh(e * 5.0), np.tanh(e_sq * 2.0),
                              np.tanh(de * 5.0), np.tanh((e - 2 * last_e) * 5.0),
                              np.tanh(np.log1p(input_norm / M_eff + 1e-8)),
                              np.tanh(np.log1p(e_sq + 1e-8)),
                              autocorr,
                              np.tanh((last_mu_mean - 0.5) * 4.0),
                              np.tanh((last_lam - 0.85) * 10.0),
                              np.tanh(abs(last_mu_mean * e) / np.sqrt(input_norm + 1e-8) * 2.0),
                              np.sign(de),
                             ], dtype=torch.float32, device=device),
                torch.tanh(w_snippet * 0.1),
            ...
            w = lam[0] * w + (mu_expanded / input_norm) * e * x_exp
