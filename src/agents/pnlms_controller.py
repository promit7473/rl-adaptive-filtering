"""PNLMS Controller: LSTM with per-tap step-size output + auxiliary heads.

Action space: M+1 continuous values
  - mu_t[0..M-1]: per-tap step sizes (proportionate NLMS)
  - lam_t: global leakage factor

This is the key architectural change over the scalar-mu HybridController.
By outputting per-tap step sizes, the controller can learn to focus
adaptation energy on taps corresponding to the active noise regime.

Feature space: 11 + M (base 11 features + current weight vector snippet)
The weight vector snippet lets the controller see which taps are already
well-adapted and which need more correction.
"""
...
class PNLMSController(nn.Module):
    """LSTM controller with per-tap step-size output and auxiliary heads.

    Heads:
      aux_error  : predict e_{t+1} — world model of filter dynamics
      aux_signal : predict d_{t+1} — world model of clean signal
      aux_task   : classify noise family — task inference
    """
    def __init__(self, feat_dim: int = 27, hidden: int = 256,
